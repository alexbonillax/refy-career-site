...
export const Accordion = ({ first, children, title, iconName, open = true, setHeight }: AccordionProps) => {
  const targetRef = useRef(null);
  const [state, setState] = useState(open);
  const [maxHeight, setMaxHeight] = useState('unset');
  const toggle = () => setState(!state);
  useEffect(() => {
    setMaxHeight(setHeight ? setHeight : targetRef.current.clientHeight);
  }, []);
  return (
    <div className="">
      {
